...
One of the main features of the SpotGraph package is to identify spots on a slide that lie on top of tissue debris disconnected from the rest of the tissue sample. These are uninformative and are likely technical artifacts from sample processing, so we'd like to exclude these from our downstream analysis. We can count the number of spots surrounding each spot using the `igraph::degree()` function, and identify spots that have no neighbors or only have one adjacent spot:
